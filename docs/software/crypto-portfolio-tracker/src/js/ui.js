...
import { getPortfolio, addOrUpdateCoin, removeCoin, clearPortfolio, setPortfolio, recordAction } from './portfolio.js';
...
async function populateCoinDatalist(force = false) {
  ...
  try {
    const list = await fetchCoinList(force);
    ...
    if (els.coinListInfo) {
      els.coinListInfo.textContent = `Loaded ${items.length} symbols${force ? ' (refreshed)' : ''}.`;
    }
  } catch (e) {
        ...
        <tbody>
          ${actions.map(a => {
            ...
            return `<tr>
              ...
              <td class="px-2 py-0.5 text-right">${a.quantity}</td>
              <td class="px-2 py-0.5 text-right">${a.price != null ? '€'+a.price.toFixed(2):'—'}</td>
              <td class="px-2 py-0.5 text-right">${a.valueEUR != null ? '€'+a.valueEUR.toFixed(2):'—'}</td>
              <td class="px-2 py-0.5 text-right ${rpCls}">${rp}</td>
              <td class="px-2 py-0.5">${a.date}</td>
            </tr>`;
          }).join('')}
          <tr class="font-medium">
            <td class="px-2 py-1" colspan="4">Realized Total</td>
            <td class="px-2 py-1 text-right ${realizedTotal>=0?'text-bull':'text-bear'}">${realizedTotal ? (realizedTotal>=0?'+':'')+'€'+realizedTotal.toFixed(2) : '—'}</td>
            <td></td>
          </tr>
        </tbody>
